...
class _EntregadorDetalhesModalState extends State<EntregadorDetalhesModal> {
      ...
      children: [
        Container(
          ...
          child: Text(
            ...
            style: GoogleFonts.dmSans(
                fontSize: 11, color: const Color(0xFF3B82F6), height: 1.6),
          ),
        ),
        const SizedBox(height: 12),
        if (selfie == null)
          ...
          Builder(builder: (_) {
            final cfg =
                selfieStCfg[selfie.status] ?? selfieStCfg['revisao_manual']!;
            return Container(
              padding: const EdgeInsets.all(14),
              decoration: BoxDecoration(
                color: cfg.$2,
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(
              child: ClipRRect(
                ...
                child: LinearProgressIndicator(
                  value: e.docTotal > 0 ? e.docCount / e.docTotal : 0.0,
                  minHeight: 6,
                  backgroundColor: e.docCount == e.docTotal
            ...
            Text('${e.docCount}/${e.docTotal} enviados',
                style: GoogleFonts.dmSans(
                    fontSize: 11,
                    fontWeight: FontWeight.w700,
                    color: e.docCount == e.docTotal
        ...
        const SizedBox(height: 12),
        ...e.docTiposVisiveis.map((tipo) {
          final ok = e.docEnviado(tipo);
          final obrigatorio = e.docObrigatorio(tipo);
          final validacao =
              e.docs[tipo]; // 'pendente' | 'aprovado' | 'reprovado' | null
  ...
  Widget _infoGridCustom(List<(String, String, bool)> fields) {
    return GridView.count(
      ...
      children: fields.map((f) => _infoCell(f.$1, f.$2, alert: f.$3)).toList(),
    );
  }
