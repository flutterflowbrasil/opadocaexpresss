...
class _ProdutoFormModalState extends ConsumerState<_ProdutoFormModal>
  ...
  /// false quando a categoria selecionada for de bebidas (sucos, refrigerantes, etc.)
  bool _categoriaPermiteUltimaMordida = true;
  ...
  int get _tabCount =>
      3 +
      (_categoriaPermiteAdicionais ? 1 : 0) +
      (_categoriaPermiteUltimaMordida ? 1 : 0);
  ...
  void initState() {
    ...
    if (_isEdicao) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        if (mounted) setState(_recalcularPermissoesDeCat);
  ...
  Future<void> _excluirProduto() async {
    final conf = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Tem certeza que deseja excluir "${widget.produto!.nome}"? Esta ação não pode ser desfeita e removerá permanentemente o produto do cardápio.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(ctx).pop(false),
            child: Text('Cancelar', style: GoogleFonts.publicSans(color: Colors.grey.shade600)),
          ),
          ElevatedButton(
            ...
            child: Text('Excluir', style: GoogleFonts.publicSans(fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
    ...
    if (conf == true) {
      ...
      try {
        ...
        if (mounted) {
          _mostrarToast('Produto excluído com sucesso!');
          Navigator.of(context).pop();
        }
      } catch (e) {
        if (mounted) {
          _mostrarToast('Erro ao excluir produto: $e', isError: true);
        }
      } finally {
        if (mounted) {
          setState(() => _isSaving = false);
  ...
  /// Retorna true se a categoria for de bebidas — oculta a aba Última Mordida.
  bool _ehCategoriaDeBebidas(CategoriaEstabelecimentoModel? cat) {
    if (cat == null) return false;
    final slug = cat.slug.toLowerCase();
    final nome = cat.nome.toLowerCase();
    const slugsExatos = [
      'bebida', 'bebidas', 'suco', 'sucos', 'refrigerante', 'refrigerantes',
      'drink', 'drinks', 'agua', 'aguas', 'cerveja', 'cervejas', 'chopp',
      'chopps', 'leite', 'vitaminas', 'vitamina', 'smoothies', 'smoothie',
      'isotonicos', 'energeticos', 'cafes', 'cafe', 'chas', 'cha',
      'limonadas', 'limonada', 'milkshakes', 'milkshake',
    ];
    const termosNome = [
      'bebid', 'suco', 'refriger', 'drink', 'água', 'agua', 'cerveja',
      'chopp', 'caipir', 'coquet', 'smoothie', 'vitamina', 'isotônic',
      'isotonic', 'energétic', 'energetic', 'limonada', 'milkshake',
      'cappuccin', 'espresso',
    ];
    if (slugsExatos.any((s) => slug == s || slug.startsWith('$s-'))) return true;
    return termosNome.any((t) => nome.contains(t));
  }

  /// Verifica o nome de uma categoria de cardápio do estabelecimento (fallback).
  bool _ehNomeDeBebidasCardapio(String nome) {
    final n = nome.toLowerCase();
    const termos = [
      'bebid', 'suco', 'refriger', 'drink', 'água', 'agua', 'cerveja',
      'chopp', 'caipir', 'coquet', 'smoothie', 'vitamina', 'isotônic',
      'isotonic', 'energétic', 'energetic', 'limonada', 'milkshake',
      'cappuccin', 'espresso', 'leite', 'cha ', 'chá ', 'café ', 'cafe ',
    ];
    return termos.any((t) => n.contains(t)) ||
        n.trim() == 'cha' ||
        n.trim() == 'chá' ||
        n.trim() == 'cafe' ||
        n.trim() == 'café';
  ...
  void _atualizarTabController() {
    ...
    if (_tabController.length != novoCount) {
      final indexAtual = _tabController.index;
      _tabController.dispose();
      _tabController = TabController(
        length: novoCount,
        vsync: this,
        initialIndex: indexAtual.clamp(0, novoCount - 1),
      );
    }
  }
  ...
  void _recalcularPermissoesDeCat() {
    ...
    // 2. Fallback: verifica pelo nome da categoria do cardápio do
                ...
                Container(
                  ...
                  child: TabBar(
                    ...
                    tabs: [
                      ...
                      if (_categoriaPermiteUltimaMordida)
                ...
                Expanded(
                  ...
                  child: TabBarView(
                    ...
                    children: [
                      ...
                      // 4. Opções (oculta para categorias sem adicionais)
                      ...
                      // 5. Última Mordida (oculta para bebidas)
...
class _TabUltimaMordidaState extends ConsumerState<_TabUltimaMordida> {
  ...
  /// Override local após ação — null = usa snapshot do produto
  ...
  /// Usa estado local (pós-ação) ou snapshot do produto

  bool get _estaAtivo => _estadoLocal ?? (widget.produto?.ultimaMordida ?? false);
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Icon(
                      ...
                      color: _feedbackIsError ? Colors.red.shade600 : Colors.green.shade700,
                    ...
                    : () async {
                        ...
                        setState(() {
                          ...
                          if (err == null) {
                            ...
                            _feedbackMsg = '✓ Última Mordida ativada com sucesso!';

                          } else {

                            _feedbackIsError = true;

                            _feedbackMsg = 'Erro ao ativar: $err';
            ...
            if (_feedbackMsg != null)
              ...
              AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  color: _feedbackIsError ? Colors.red.shade50 : Colors.blue.shade50,

                  borderRadius: BorderRadius.circular(10),

                  border: Border.all(

                    color: _feedbackIsError ? Colors.red.shade300 : Colors.blue.shade300,
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Icon(
                      ...
                      _feedbackIsError ? Icons.error_outline : Icons.info_outline,

                      color: _feedbackIsError ? Colors.red.shade600 : Colors.blue.shade700,
                    ...
                    : () async {
                        ...
                        setState(() {
                          ...
                          if (err == null) {
                            ...
                            _feedbackMsg = 'ℹ️ Última Mordida desativada.';

                          } else {

                            _feedbackIsError = true;

                            _feedbackMsg = 'Erro ao desativar: $err';
