-- Checkout falhava com numeric field overflow (22003):
-- pedidos.distancia_km era numeric(5,2) (max 999.99).

ALTER TABLE public.pedidos
  ALTER COLUMN distancia_km TYPE numeric(8,2);

-- Distancia absurda (geo invalida, ex. lat/lng invertidos) vira 0 e nao infla a taxa.
CREATE OR REPLACE FUNCTION public.fn_distancia_km_coords(
...
AS $$
  SELECT CASE
    WHEN p_lat1 IS NULL OR p_lng1 IS NULL OR p_lat2 IS NULL OR p_lng2 IS NULL THEN 0
    WHEN abs(p_lat1) > 90 OR abs(p_lat2) > 90 OR abs(p_lng1) > 180 OR abs(p_lng2) > 180 THEN 0
    WHEN ROUND((111.045 * degrees(acos(LEAST(1.0, GREATEST(-1.0,
      cos(radians(p_lat1)) * cos(radians(p_lat2))
        * cos(radians(p_lng2) - radians(p_lng1))
        + sin(radians(p_lat1)) * sin(radians(p_lat2))
    )))))::numeric, 2) > 200 THEN 0
    ELSE ROUND((111.045 * degrees(acos(LEAST(1.0, GREATEST(-1.0,
      cos(radians(p_lat1)) * cos(radians(p_lat2))
        * cos(radians(p_lng2) - radians(p_lng1))
        + sin(radians(p_lat1)) * sin(radians(p_lat2))
    )))))::numeric, 2)
