-- Checkout: usa distância de rota (Google Directions) quando sane;
-- deixa de rejeitar pedido por raio_maximo_km (aviso só no app).
-- raio_maximo_km permanece para despacho de entregadores.

CREATE OR REPLACE FUNCTION public.fn_distancia_entrega_km(
  p_haversine_km numeric,
  p_distancia_rota_km numeric DEFAULT NULL
)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT CASE
    WHEN p_distancia_rota_km IS NULL THEN COALESCE(p_haversine_km, 0)
    WHEN p_distancia_rota_km <= 0 OR p_distancia_rota_km > 200 THEN COALESCE(p_haversine_km, 0)
    WHEN p_distancia_rota_km > (COALESCE(p_haversine_km, 0) * 1.5 + 2) THEN COALESCE(p_haversine_km, 0)
    ELSE ROUND(p_distancia_rota_km, 2)
  END;
$$;

DROP FUNCTION IF EXISTS public.criar_pedido_validado(uuid, uuid, text, text, text);
DROP FUNCTION IF EXISTS public.criar_pedido_validado(uuid, uuid, text, text, text, numeric);

CREATE FUNCTION public.criar_pedido_validado(
...
BEGIN
  ...
  v_distancia := public.fn_distancia_entrega_km(
    public.fn_distancia_km_coords(
...
GRANT EXECUTE ON FUNCTION public.criar_pedido_validado(uuid, uuid, text, text, text, numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_distancia_entrega_km(numeric, numeric) TO authenticated;
